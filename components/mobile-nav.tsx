    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="md:hidden hover:bg-beige-light/10 transition-colors duration-300"
        >
      ...
      <SheetContent side="left" className="w-[300px] sm:w-[350px]">
        <div className="flex flex-col h-full">
          ...
          <nav className="flex flex-col gap-8">
            {[
              ...
              <Link
                key={item.href}
                href={item.href}
                className={`text-lg font-medium hover:text-beige-light transition-colors animate-slide-right ${item.delay} py-2`}
          ...
          <div className="mt-auto pt-6 animate-slide-up animation-delay-500">
            <Button
              className="w-full bg-beige-light hover:bg-legal-accent-dark transition-transform duration-300 hover:scale-105"
              onClick={handleLinkClick}
            >
              {t("nav.getStarted")}
            </Button>
